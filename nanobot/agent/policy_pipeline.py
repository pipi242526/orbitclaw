...
class PolicyPipeline:
    ...
    _TOOL_TRACE_LINE_PATTERNS: tuple[re.Pattern[str], ...] = (
        re.compile(r"^\s*↳\s+"),
        re.compile(r"^\s*🐈\s*nanobot\s*$", re.IGNORECASE),
    ...
    def help_text_from_specs(
        ...
        *,
        ...
        title = "🐈 nanobot 命令：" if zh else "🐈 nanobot commands:"
        rows = [title]
    ...
    def format_user_error(
        ...
        *,
        ...
        if "tool '" in lower and "not found" in lower:
            ...
            fixes.extend([
                t(
                    "Run `nanobot doctor` to inspect tool/skill dependency and config issues.",
