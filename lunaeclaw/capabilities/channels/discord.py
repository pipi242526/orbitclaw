...
class DiscordChannel(BaseChannel):
        ...
        for attempt in range(3):
            try:
                ...
                if response.status_code == 429:
                    data = response.json()
                    retry_after = clamp_retry_after(data.get("retry_after"), default=1.0)
                ...
                if attempt == 2:
                    ...
                    await asyncio.sleep(linear_retry_delay(1.0, attempt, step_seconds=1.0, maximum=3.0))
        return False
    ...
    async def _handle_message_create(self, payload: dict[str, Any]) -> None:
        ...
        def _sanitize_component(raw: Any, default: str) -> str:
            text = safe_filename(str(raw))
            # Keep file names path-safe across platforms and avoid dot-segment tricks.
        ...
        for attachment in payload.get("attachments") or []:
            ...
            if size and size > MAX_ATTACHMENT_BYTES:
                ...
                continue
            try:
                attach_id = _sanitize_component(attachment.get("id", "file"), "file")
                safe_name = _sanitize_component(filename, "attachment")
                file_path = media_dir / f"{attach_id}_{safe_name}"
